...
#get tips for two asterace substrices,
aster<-fam %>% 
  filter(family=='Asteraceae') %>% 
  separate(species_matched, into=c('genus', 'species'), remove = F)

card<-aster %>% 
  filter(genus %in% c("Arctium", "Carduus", "Carlina", "Carthamus", "Centaurea",  "Cirsium", "Cnicus", "Cynara", "Echinops", "Galactites","Jurinea", "Notobasis", "Onopordum", "Rhaponticum", "Saussurea", "Serratula", "Silybum", "Staehelina", "Xeranthemum", "Alfredia", "Amberboa", "Ancathia", "Atractylis", "Berardia", "Cardopatium","Carduncellus", "Chardinia", "Cousinia", "Crupina", "Dolomiaea", "Femeniasia", "Klasea", "Lamyropsis", "Leuzea", "Mantisalca","Myopordon", "Nikitinia", "Oligochaeta", "Olgaea", "Phonus","Picnomon", "Plagiobasis", "Plectocephalus", "Psephellus",
"Ptilostemon", "Rhaponticoides", "Russowia", "Schischkinia",
"Schmalhausenia", "Siebera", "Stizolophus", "Synurus", "Takeikadzuchia", "Tricholepis", "Tyrimnus", "Volutaria", "Zoegea"))
...
tips4<-card$species_matched
tips5<-gnaph$species_matched
...
tree2 <- groupClade(tree, c(list.nod1$num, 2347, 2147)) #the extra nodes are from Asteraceae 
