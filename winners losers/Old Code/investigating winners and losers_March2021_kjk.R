...
#investigate winners losers -- investigating whether gains/losses are messing up patterns
...
my.wd <- "C:\\Users\\lapie\\Dropbox (Smithsonian)\\working groups\\CoRRE\\sDiv\\sDiv_sCoRRE_shared\\"
...
CT_diff<-CT%>%
  mutate(diff=treatDCi-DCi,
         absdiff=abs(diff))%>%
  mutate(gain_loss=ifelse(treatDCi==0, 'loss', ifelse(DCi==0, 'gain', 'neither')))
...
#LRR
CT_LRR<-CT%>%
  mutate(lrr=log((treatDCi+0.01)/(DCi+0.01)))%>%
  mutate(gain_loss=ifelse(treatDCi==0, 'loss', ifelse(DCi==0, 'gain', 'neither')))

#grman's E
CT_E<-CT%>%
  mutate(E=(treatDCi-DCi)/(treatDCi+DCi))%>%
  mutate(gain_loss=ifelse(treatDCi==0, 'loss', ifelse(DCi==0, 'gain', 'neither')))

###how many species are 0's for either DCi or treatDCi?
trtDCiZero <- CT%>%filter(treatDCi==0) #334 of 34240 observations (0.9%)
ctlDCiZero <- CT%>%filter(DCi==0) #7409 of 34240 observations (21.6%)

trtDCiZeroHist <- ggplot(data=trtDCiZero, aes(x=DCi)) + geom_histogram() + ggtitle('Not in Treatment\n(0.9% of cases)') + coord_cartesian(xlim=c(0,0.8), ylim=c(0,1500))
ctlDCiZeroHist <- ggplot(data=ctlDCiZero, aes(x=treatDCi)) + geom_histogram() + ggtitle('Not in Control\n(21.6% of cases)') + coord_cartesian(xlim=c(0,0.8), ylim=c(0,1500))

grid.arrange(trtDCiZeroHist, ctlDCiZeroHist, ncol=2)


#where do these fall within the difference?
#abs diff
gplot(data=CT_diff, aes(x=absdiff, fill=gain_loss)) +
  geom_histogram() +
  scale_fill_manual(values=c('#FC766AFF', '#B0B8B4FF', '#184A45FF'))

#just diff
ggplot(data=CT_diff, aes(x=diff, fill=gain_loss)) +
  geom_histogram() +
  scale_fill_manual(values=c('#FC766AFF', '#B0B8B4FF', '#184A45FF'))

#LRR
ggplot(data=CT_LRR, aes(x=lrr, fill=gain_loss)) +
  geom_histogram() +
  scale_fill_manual(values=c('#FC766AFF', '#B0B8B4FF', '#184A45FF'))

#grman's E
ggplot(data=CT_E, aes(x=E, fill=gain_loss)) +
  geom_histogram() +
  scale_fill_manual(values=c('#FC766AFF', '#B0B8B4FF', '#184A45FF'))
...
#how does abundance, and freqency in control plots affect difference?
