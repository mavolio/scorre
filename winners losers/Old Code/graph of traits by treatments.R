...
dcidiff_models<-read.csv("E:/Dropbox/sDiv_sCoRRE_shared/WinnersLosers paper/data/Species_DCiDiff_formixedmodels.csv")
...
alldat_cont<-dcidiff_models%>%
  ...
  filter(trait=="SRL", trt_type2=="n"|trt_type2=="all mult"|trt_type2=="p")
...
m1<-lmer(diff ~ -1 + trt_type2 + value:trt_type2 + (trt_type2|species_matched) + (trt_type2|site_code), data=alldat_cont)
...
plot<-as.data.frame(summary(m1)$coefficients)
fixef(m1) #should give fixed effects of model


alldat_cat<-dcidiff_mult_n%>%
  right_join(traitsScaled)%>%
...
m2<-lmer(diff ~ -1 + trt_type2 + value:trt_type2 + (trt_type2|species_matched) + (trt_type2|site_code), data=alldat_cat)
