...
#create a heat map of DCI responses to label species tips

heat<-gheatmap(p, dat, offset=1.5, width=0.03, colnames = F, color=NULL)+
  scale_fill_scico(palette = "vik", direction=-1, na.value="gray48", limits=c(-0.6,0.6))+
  theme(legend.position = "bottom", legend.text = element_text(size=15), legend.title=element_text(color = "white"))
