...
trait_syndrome<-alldat_cat_full %>%
  pivot_wider(names_from=trait, values_from = trait_value2, values_fill = NA) %>% 
  select(species, growth_form, photosynthetic_pathway, lifespan, clonal, mycorrhizal_type, n_fixation_type) %>% 
  unique()  

c4test<-TraitSyndrome %>% 
  filter(syndrome=='C4/CAM')
 
c4<-trait_syndrome %>% 
  mutate(syndrome=ifelse(photosynthetic_pathway=="C4/CAM", 'C4/CAM', 'drop')) %>% 
  filter(syndrome!='drop')


anforb<-trait_syndrome %>% 
  mutate(syndrome=ifelse(growth_form=='forb'&lifespan=='annual', 'Annual forb', 'drop')) %>% 
  filter(syndrome!='drop')

angram<-trait_syndrome %>% 
  mutate(syndrome=ifelse(growth_form=='graminoid'&lifespan=='annual', 'Annual gram.', 'drop')) %>% 
  filter(syndrome!='drop')

nomyc<-trait_syndrome %>% 
  mutate(syndrome=ifelse(growth_form=='forb'&mycorrhizal_type=='none'&n_fixation_type=='none', 'Non-mutualistic forb', 'drop')) %>% 
  filter(syndrome!='drop')
         
nfix<-trait_syndrome %>% 
  mutate(syndrome=ifelse(growth_form=='forb'&n_fixation_type=='N-fixer', 'N-fixing forb', 'drop')) %>% 
  filter(syndrome!='drop')

myc<-trait_syndrome %>% 
  mutate(syndrome=ifelse(growth_form=='forb'&mycorrhizal_type=='yes','Mycorrhizal forb', 'drop')) %>%
  filter(syndrome!='drop')
          
nonclonal<-trait_syndrome %>% 
  mutate(syndrome=ifelse(growth_form=='forb'&clonal=='no', 'NoClon', 'drop')) %>% 
  filter(syndrome!='drop')

woody<-trait_syndrome %>% 
  mutate(syndrome=ifelse(growth_form=='woody', 'Woody', 'drop')) %>% 
  filter(syndrome!='drop')
      
pgrass<-trait_syndrome %>% 
  mutate(syndrome=ifelse(growth_form=='graminoid'&lifespan=='Perenn./Bienn.', 'Peren. Gram.', 'drop')) %>% 
  filter(syndrome!='drop')

TraitSyndrome1<-c4 %>% 
  bind_rows(anforb, angram, nomyc, nfix, woody)

SyndromeN<-TraitSyndrome1 %>% 
  group_by(syndrome) %>% 
  summarise(n=length(species))

SpSyndrome1<-TraitSyndrome1 %>% 
  select(species, syndrome) %>% 
  filter(syndrome!='todo')

syndrome_diff_all1 <-dcidiff_models %>% 
  left_join(SpSyndrome1)


###This give the number of obs for the figure
tmp = syndrome_diff_all1
tmp = tmp[!is.na(tmp$diff),]
synd_n_data1 = table(unique(tmp[,c("species", "trt_type2", "syndrome")]))
synd_n_data1 = apply(synd_n_data1, 2:3, sum)

value2 = colnames(synd_n_data1)

#syndromes
msynd2<-lmer(diff ~ -1 + trt_type2 + syndrome:trt_type2 + (1|species) + (1|site_code), data=subset(syndrome_diff_all1, syndrome=='C4/CAM'))

plot.msynd1<-as.data.frame(emmeans(msynd1, ~ syndrome*trt_type2))%>%
  mutate(trait="Synd") %>% 
...
pdf("syndromes2_by_treat_contMay2025.pdf", width = 6, height=10)
