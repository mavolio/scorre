...
#this function creates dataframes for different treatment types that include both target treatment and control plots
...
#this function is for site vote counting
...
#pulls coefficients out of across-sites mixed-effects models
...
#cleaning PD data to bind with treatment data
###some of this can be removed if periods aren't used for separation
...
#merge PD data with treatment data
...
#make dataframes for each treatment
...
#Double check comparison with fixed effects approach (econometrics)
...
#organize model outputs into table
...
out_2$trt_type<- c(rep("nut",18),rep("drought",18), rep("CO2",18), rep("dist",18), rep("irg",18), rep("herb_removal",18), rep("temp",18))
out_2$pd.metric <- rep(c("pd.raw", "pd.raw","pd.raw","pd.ses", "pd.ses","pd.ses","mdp.raw", "mdp.raw","mdp.raw","mpd.ses", "mpd.ses","mpd.ses","mntd.raw", "mntd.raw","mntd.raw","mntd.ses","mntd.ses","mntd.ses"),7)
