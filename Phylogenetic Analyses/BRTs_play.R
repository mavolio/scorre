...
my.wd <- "C:/Users/mavolio2/Dropbox/sDiv_sCoRRE_shared/paper 2_PD and FD responses/data/"
...
ggplot(data=nit, aes(x=n, y=mpd_diff_avg))+
  geom_point()

ggplot(data=nit, aes(x=MAP, y=mpd_diff_avg))+
  geom_point()

ggplot(data=nit, aes(x=rrich, y=mpd_diff_avg))+
  geom_point()

ggplot(data=nit, aes(x=anpp, y=mpd_diff_avg))+
  geom_point()

ggplot(data=nit, aes(x=MAT, y=mpd_diff_avg))+
  geom_point()

summary(m1<-lm(mpd_diff_avg~n+MAP+rrich+anpp+MAT, data=nit))
...
mod <-  gbm.step(data=nit, gbm.x = c("MAP", "MAT", "rrich", "anpp", "n"), 
...
hist(mod$residuals)
mod$contributions
cor(nit$mpd_diff_avg, mod$fitted)^2 #pseudo-R2
...
png("mpd_nit_pdp.png",res=600,height=7,width=7,units="in"); 
