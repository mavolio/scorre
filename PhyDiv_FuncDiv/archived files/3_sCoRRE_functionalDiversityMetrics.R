...
setwd("C:\\Users\\kjkomatsu\\Smithsonian Dropbox\\Kimberly Komatsu\\working groups\\CoRRE\\CoRRE_database\\Data") # Kim's laptop/desktop
...
# trait data
traitsCategorical <- read.csv('https://portal.edirepository.org/nis/dataviewer?packageid=edi.1533.3&entityid=5ebbc389897a6a65dd0865094a8d0ffd') %>% 
  select(-error_risk_overall, -source) %>% 
  #what error risk is too much and should be dropped?
  pivot_wider(names_from=trait, values_from=trait_value)

traitsContinuous <- read.csv('https://portal.edirepository.org/nis/dataviewer?packageid=edi.1533.3&entityid=169fc12d10ac20b0e504f8d5ca0b8ee8') %>% 
  select(-imputation_error, -error_risk_overall, -error_risk_family, -error_risk_genus, -source) %>% 
  #what error risk is too much and should be dropped?
  pivot_wider(names_from=trait, values_from=trait_value) 

traits <- full_join(traitsCategorical, traitsContinuous) %>% 
  ...
  drop_na() #only keep trait data that is complete for all traits (drops 1300 species with only categorical trait data, 2851 remaining)
...
colnames(traitsScaled) <- c('species', 'growth_form', 'photosynthetic_pathway', 'lifespan', 'clonal', 'mycorrhizal_type', 'n_fixation_type', 'LDMC', 'SLA', 'leaf_N', 'SRL', 'plant_height_vegetative', 'seed_dry_mass')
...
relCovClean <- relCoverRaw %>%
  ...
  mutate(species2=ifelse(is.na(species), genus_species, species)) %>% 
  select(-genus_species, -species) %>% 
  rename(species=species2) %>% 
  group_by(site_code, project_name, community_type, site_proj_comm, calendar_year, treatment_year, treatment, block, plot_id, species) %>% 
...
for(s in 1:length(site_vector)){
  ...
  relCoverSubset <- relCovClean %>%
    ...
    mutate(relcov2=ifelse(relcov>0, 1, 0)) %>% #convert to presence/absence
    select(-relcov) %>% 
  ...
  sppSubset <- data.frame(species = unique(relCoverSubset$species), dummy=1) %>%
    # left_join(corre_to_try, by="species") %>%
    unique() 
  ...
  traitsSubsetArranged <- traitsSubset %>%
    ...
    mutate_at(.vars=c("growth_form", "photosynthetic_pathway","lifespan", "clonal", "mycorrhizal_type", "n_fixation_type"), 
              funs(as.numeric(as.factor(.)))) %>%
    mutate_at(.vars=c("leaf_N", "LDMC", "SLA", "plant_height_vegetative", "SRL", "seed_dry_mass"), 
              funs(as.numeric(.))) %>% 
  ...
  traitMatrixSubset <- as.matrix(gawdis(traitsSubsetArranged, w.type = "optimized", opti.maxiter = 200, 
                                        groups.weight=T, groups = c(1,2,3,4,5,6,7,7,7,8,9,10)))
...
# write.csv(functionalDiversityMetrics, 'C:\\Users\\kjkomatsu\\Smithsonian Dropbox\\Kimberly Komatsu\\working groups\\CoRRE\\sDiv\\sDiv_sCoRRE_shared\\paper 2_PD and FD responses\\data\\CoRRE_functionalDiversity_20241118.csv',row.names=F)