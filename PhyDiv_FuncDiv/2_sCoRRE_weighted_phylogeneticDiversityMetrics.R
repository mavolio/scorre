...
phylogeneticDiversityMetricsFull <- NULL
...
for (i in 1:length(sites)){ #loop to calculate metrics for each site independently
  ...
  #bind values into RR dataframe
  phylogeneticDiversityMetricsFull <- rbind(phylogeneticDiversityMetricsFull, mpdSubsetSES)
  
}


#### combine subset and full metrics ####
phylogeneticDiversityMetricsComplete <- phylogeneticDiversityMetricsFull %>% 
  full_join(phylogeneticDiversityMetrics)

#save output:
saveRDS(phylogeneticDiversityMetricsComplete, file = "PhyDiv_FuncDiv/phylogeneticDiversityMetrics.rds")
